...
const DefaultBaseURL = "https://api.ant-ling.com/v1"

// Provider adapts Ant Ling's OpenAI-compatible Chat Completions endpoint.
type Provider = openai.Provider

// ProviderOption configures a Provider.
type ProviderOption = openai.ProviderOption

// NewProvider constructs an Ant Ling provider.
...
// Register adds an Ant Ling text provider to registry.
func Register(registry *sigma.Registry, opts ...ProviderOption) error {
	...
	return registry.RegisterTextProvider(sigma.ProviderAntLing, NewProvider(opts...))
}

// RegisterDefault adds an Ant Ling text provider to sigma's default registry.
func RegisterDefault(opts ...ProviderOption) error {
	return sigma.RegisterDefaultTextProvider(sigma.ProviderAntLing, NewProvider(opts...))
}
